package ediger.diarynutrition.data.source.dao;

import java.util.List;

import androidx.lifecycle.LiveData;
import androidx.room.Dao;
import androidx.room.Insert;
import androidx.room.OnConflictStrategy;
import androidx.room.Query;
import androidx.room.Update;
import ediger.diarynutrition.data.source.entities.Food;

@Dao
public interface FoodDao {

    @Query("SELECT * FROM food ORDER BY food_name ASC")
    LiveData<List<Food>> getAllFood();

    @Query("SELECT * FROM food WHERE favorite = 1 ORDER BY food_name ASC")
    LiveData<List<Food>> getFavorFood();

    @Query("SELECT * FROM food WHERE user = 1 ORDER BY food_name ASC")
    LiveData<List<Food>> getUserFood();

    @Query("SELECT * FROM food WHERE id = :id")
    Food getFood(int id);

    @Insert(onConflict = OnConflictStrategy.IGNORE)
    void insertFood(Food food);

    @Update
    void updateFood(Food food);

    @Query("DELETE FROM food WHERE id = :id")
    int deleteFoodById(int id);

    @Query("SELECT * FROM food " +
            "WHERE food_name LIKE :text " +
            "ORDER BY food_name = :text DESC, food_name LIKE :text DESC ")
    LiveData<List<Food>> searchAllFood(String text);

    @Query("SELECT * FROM food " +
            "WHERE favorite = 1 AND food_name LIKE :text " +
            "ORDER BY food_name = :text DESC, food_name LIKE :text DESC ")
    LiveData<List<Food>> searchFavorFood(String text);

    @Query("SELECT * FROM food " +
            "WHERE user = 1 AND food_name LIKE :text " +
            "ORDER BY food_name = :text DESC, food_name LIKE :text DESC ")
    LiveData<List<Food>> searchUsrFood(String text);

    //FTS4?
}
